...
CashflowSc.prototype.hasShutter = () => false
...
CashflowSc.prototype.run = function run (cb, _) {
  this.open(() => {
...
CashflowSc.prototype._parse = function _parse (packet) {
  ...
  // For escrow, need to emit both billsAccepted and billsRead
  if (status === 'billsRead') {
    ...
    return process.nextTick(() => this.emit('billsRead', [result.bill]))
  }

  // This can happen when cashbox is re-inserted
  if (status === 'billsValid' && result.bill && !result.bill.denomination) return
...
function parseStatus (data) {
  return data[0].stacked ? 'billsValid' :
    data[0].escrowed ? 'billsRead' :
    // NOTE: A billsAccepted event is emitted right before a billsRead so there's
    // no need to return billsAccepted here.
    //(data[0].accepting || data[0].stacking) ? 'billsAccepted' :
...
bv.on('billsRead', function(denomination) {
