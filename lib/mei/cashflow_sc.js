...
function parseStatus (bytes) {
  ...
  const want = ([idx, mask, status]) => (bytes[idx] & mask) === mask
  const status = STATUS_MASKS.find(want)
  return status ? status[2] : null
}

function parseStandard (statusBytes) {
  const getNth = (b, n) => Boolean((b >> n) & 0b1)
  const getNths = (b, f, t) => {
    const n = t - f
    const mask = ~((~0) << n)
    return (b >> f) & mask
  ...
  const B0 = statusBytes[0]
  const B0st = {
    idling:    getNth(B0, 0), // Not processing a document
    accepting: getNth(B0, 1), // Drawing a document in
    escrowed:  getNth(B0, 2), // Valid document in escrow
    stacking:  getNth(B0, 3), // Stacking valid document
    stacked:   getNth(B0, 4), // Stacked valid document
    returning: getNth(B0, 5), // Returning document to customer
    returned:  getNth(B0, 6), // Returned document to customer
  }

  const B1 = statusBytes[1]
  const B1st = {
    cheated:          getNth(B1, 0), // Validator thinks customer tried to fraud the system
    rejected:         getNth(B1, 1), // Document was rejected and returned to customer
    jammed:           getNth(B1, 2), // Validator is jammed
    stackerFull:      getNth(B1, 3), // Cashbox is full
    cassetteAttached: getNth(B1, 4), // Is cashbox inside the validator?
    paused:           getNth(B1, 5), // Customer trying to feed document while validator is processing another document
    calibrating:      getNth(B1, 6), // Is validator calibrating?
  }

  const B2 = statusBytes[2]
  const B2st = {
    powerup:        getNth(B2, 0), // Is validator powering up? Not ready to accept documents if yes
    invalidCommand: getNth(B2, 1), // Validator received an invalid command
    failure:        getNth(B2, 2), // Validator ist kaput!
    noteValue:      getNths(B2, 3, 6), // Only valid when in non-extended mode AND escrowed^stacked
    transportOpen:  getNth(B2, 6), // ???
  }

  var status = parseStatus(statusBytes)
  return {
    status,
    destructed: [ B0st, B1st, B2st, ]
  }
}
...
function interpret (frame) {
  ...
  const {
    stx,
    len,
    ctl: { ack, devType, msgType, },
    data,
    etx,
    chk,
  } = destructedFrame

  return (msgType === 0b010) ? parseStandard(data) :
    (msgType === 0b111 && data[0] === 0x02) ? parseExtended(frame) :
    null
}
