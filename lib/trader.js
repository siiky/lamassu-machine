...
Trader.prototype.pollHandler = function pollHandler (data) {
  ...
  const handleDynamicConfig = data => {
    ...
    if (data.cassettes) {
      ...
      this.originalCassettes = null
      this.cassettes = null
      this.virtualCassettes = null
    }

    if (data.stackers) {
      this.originalStackers = _(data.stackers.physical)
        .map(_.update('denomination', BN))
        .value()
      this.stackers = _(data.stackers.physical)
        .orderBy(['denomination'], ['asc'])
        .map(_.update('denomination', BN))
        .value()
      this.virtualStackers = _.map(BN, data.stackers.virtual)
    } else {
      this.originalStackers = null
      this.stackers = null
      this.virtualStackers = null
    }
  ...
  return this.postPollEvents(_.pick(['reboot', 'shutdown', 'restartServices', 'emptyUnit', 'refillUnit'], data.configs.dynamic))
}
...
Trader.prototype.oldPollHandler = function oldPollHandler (res) {
  ...
  this.automaticReceiptPrintActive = res.automaticReceiptPrint
  ...
  this.machineScreenOpts = res.screenOptions
