...
const lssettings = require('./lssettings')(true)
...
Trader.prototype.pollHandler = function pollHandler (data) {
  ...
  return handleStaticConfig(data.configs.static)
    .then(_ => lssettings.saveConfig({
      coins: coinsRes.map(_.mapValues(v => v.toString())),
      locales: {
    ...
    .then(_ => handleDynamicConfig(data.configs.dynamic))
    .then(_ => handleTerms(data.terms))
    .then(_ => {
...
Trader.prototype.loadConfigsFromDB = function loadConfigsFromDB () {
  ...
  return lssettings.loadConfig()
    .then(config => {
      ...
      const { coins, staticConfig, urlsToPing, speedtestFiles, triggersAutomation, locales, terms } = config
      ...
      this.coins = _.map(_.mapValues(toBN), coins)
