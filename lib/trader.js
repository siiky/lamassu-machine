...
Trader.prototype.poll = function poll () {
  return lssettings.getVersions()
    .then(({ current, latest }) => gqlClient.query({
      query: GRAPHQL_QUERY,
      variables: {
        configVersion: current, // Tell l-s what we have, not what we know about
...
Trader.prototype.loadConfigsFromDB = function loadConfigsFromDB () {
  return lssettings.getVersionsAndConfigs()
    .then(({ configs, versions: { current, latest } }) => {
