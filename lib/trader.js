...
Trader.prototype.pollHandler = function pollHandler (data) {
  ...
  const handleStaticConfig = data => {
    ...
    lssettings.updateURLsToPing(this.urlsToPing = data.urlsToPing)
      .catch(err => console.log("Failed to update ping URLs in lssettings"))
    lssettings.updateSpeedtestFiles(this.speedtestFiles = data.speedtestFiles)
      .catch(err => console.log("Failed to update speedtest files in lssettings"))
    ...
    lssettings.updateTriggersAutomation(this.triggersAutomation = _.assign(
    ...
    )).catch(err => console.log("Failed to update triggers automation in lssettings"))
    ...
    lssettings.updateLocaleInfo(this.locale = {
    ...
    }).catch(err => console.log("Failed to update locale info in lssettings"))
...
Trader.prototype.loadConfigsFromDB = function loadConfigsFromDB () {
  const loadStaticConfig = version => lssettings.getStaticConfig(version)
    .then(({ enable_paper_wallet_only, has_lightening, server_version, two_way_mode }) => {
  ...
  const loadTerms = version => lssettings.getTerms(version)
    .then(terms => {
      if (!terms) {
        this.terms = false
        this.termsHash = null
      ...
      const { hash, title, text, accept, cancel, tcphoto, delay } = terms
      this.termsHash = hash
      this.terms = {
        active: true,
        delay: !!delay,
  ...
  return lssettings.getVersion()
    .then(
      ({ latest }) => {
        this.latestConfigVersion = latest
        return Promise.all([
          loadStaticConfig(latest),
          loadTerms(latest),
        ])
      },
      err => err === 'noversion' ? Promise.resolve() : Promise.reject(err)
    )
}
