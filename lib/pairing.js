...
function pair (totem, clientCert, connectionInfoPath, model, numOfCassettes, numOfStackers) {
    ...
    .then(({ hostname, token, identifier, caHash: expectedCaHash }) => {
      ...
      return got(`https://${hostname}:${PORT}/ca?token=${tokenHash}&id=${identifier}`, initialOptions)
        .then(r => {
          ...
          const query = querystring.stringify({ token, model, numOfCassettes, numOfStackers, id: identifier })
