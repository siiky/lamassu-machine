...
Hcm2.prototype.cashCount = function cashCount () {
  ...
  .then(body => ({
    ...
    result: _.flow(
      ...
      _.mapValues(
        _.flatMap(([noteId, _destination, count]) => repeat(count, { denomination: this.denominations[noteId] })),
      )
    )(body)
  }))
  // TODO: What to do with the other bills not in Escrow?
  .then(({ result }) => this.emit('billsRead', result.Escrow))
  .catch(res =>
    res.body.result.commonErrorCode === '84' && res.body.result.commonErrorCodeDetail === '030000' ?
      Promise.resolve({
        body: res.body,
        result: { Escrow: [] }
      }) :
...
Hcm2.prototype.dispenseByRoom = function dispenseByRoom (bills) {
  ...
  .then(body =>
    // Note shortage
    body.result.commonErrorCode === '93' && body.result.commonErrorCodeDetail === '010000' ?
