...
// TODO: move to some common module to avoid duplication
const sumBills = _.reduce((acc, value) => acc.add(value.fiat), BN(0))
function billsUpdate (bills) {
  if (!bills) return {}
...
const createBill = (bill, tx) => {
  const applyCashInFee = _.isEmpty(tx.bills) // TODO: confirm this works well with `createBills`
  ...
  return {
    id: uuid.v4(),
    fiat: BN(bill.denomination),
    fiatCode,
    cryptoCode: tx.cryptoCode,
    cashInFee,
    cashInTxsId: tx.id,
    deviceTime,
    destinationUnit: bill.destinationUnit || 'cashbox'
  }
}

const createBills = (bills, tx) => _.map(b => createBill(b.denomination, tx), bills)

function billUpdateDeprecated (bills) {
  if (!bills || _.isEmpty(bills)) return {}

  return {
    cryptoAtoms: _.reduce((acc, value) => acc.add(value.cryptoAtoms), BN(0), bills),
...
function createBillDeprecated (bill, exchangeRate, tx) {
  ...
  const fiatCode = tx.fiatCode
  const cashInFee = applyCashInFee ? tx.cashInFee : BN(0)
  const cryptoAtoms = truncateCrypto(BN(bill.denomination).div(atomRate), cryptoCode)
  const deviceTime = Date.now()
...
function computeCashOut (tx, units, virtualUnits, txLimit) {
  const denominationIsAvailable = denom =>
    !!BillMath.makeChange(units, tx.fiat.add(denom))
