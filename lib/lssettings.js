...
module.exports = (readwrite=false) => {
  ...
  const saveCustomInputChoiceList = (customInputID, choiceList) => db => {
    const insertCustomInputChoiceList1 = db => choiceText =>
      db.run(
        "INSERT INTO custom_input_choice_list (custom_input, choice_text) VALUES (?, ?)",
        customInputID, choiceText
      )
    return Promise.all(
      choiceList.map(insertCustomInputChoiceList1(db))
    )
  ...
  const loadTriggers = db => Promise.reject('unimplemented')
  ...
  const insertOperatorInfo = ({ name, phone, email, website, company_number }) => db =>
    db.run(
