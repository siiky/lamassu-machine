...
const saveStaticConfig = ({
  ...
  version, enable_paper_wallet_only, has_lightening, server_version, two_way_mode,
  country, fiatCode, primaryLocale
)

const loadStaticConfig = db => db.get(
  `SELECT version, enable_paper_wallet_only, has_lightening, server_version, two_way_mode, country, fiat_code, primary_locale
   FROM static_config
   WHERE rowid = 1`,
)
...
const saveTermsVersion = hash => db =>
...
const insertTriggersAutomation = triggersAutomation => db => {
  triggersAutomation = Object.entries(triggersAutomation).map(([t, a]) => [t, a === 'Automatic'])
  return db.run(
