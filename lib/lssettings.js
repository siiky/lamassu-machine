...
const deleteReceiptInfo = db =>
  db.run("DELETE FROM receipt_info WHERE TRUE")

const insertReceiptInfo = receiptInfo => db => {
  receiptInfo = Object.entries(receiptInfo)
  return db.run(
    "INSERT INTO receipt_info (field, enabled) VALUES " + sqlite.makeValues(2, receiptInfo.length),
    receiptInfo.flat()
  )
}

const saveReceiptInfo = receiptInfo => db =>
  deleteReceiptInfo(db)
    .then(_ => insertReceiptInfo(receiptInfo)(db))

const loadReceiptInfo = db =>
  db.all("SELECT field, enabled FROM receipt_info")
    .then(receiptInfo => receiptInfo.reduce(
      (ret, { field, enabled }) =>
...
exports.saveConfig = ({
  ...
  withTx(db => PromiseObject({
    ...
    receiptInfo: saveReceiptInfo(receiptInfo)(db),
    speedtestFiles: saveSpeedtestFiles(speedtestFiles)(db),
    staticConfig: saveStaticConfig(staticConfig, locales)(db),
...
exports.loadConfig = () =>
  withTx(db => PromiseObject({
    ...
    receiptInfo: loadReceiptInfo(db),
