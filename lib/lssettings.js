...
module.exports = (readwrite=false) => {
  ...
  const insertTrigger = db => trigger => Promise.reject('unimplemented')

  const insertTriggers = triggers => db =>
    Promise.all((triggers ?? []).map(insertTrigger(db)))

  const saveTriggers = triggers => db =>
    deleteTriggers(db)
      .then(_ => insertTriggers(triggers)(db))
