...
module.exports = (readwrite=false) => {
  ...
  const insertTerms = (hash, { title, text, accept, cancel, active, tcPhoto, delay }) => db =>
    db.run(
  ...
  const disableTerms = db => db.run("UPDATE terms SET active = FALSE WHERE TRUE")

  const saveTerms = (hash, terms) => db =>
    (!hash || !terms) ? // There are no terms
      deleteTerms(db) :
    terms.active ? // There are terms, but they've been updated
      deleteTerms(db).then(_ => insertTerms(hash, terms)(db)) :
      disableTerms(db) // There are terms but they've been disabled
  ...
  const saveOperatorInfo = operatorInfo => db =>
    deleteOperatorInfo(db)
      .then(_ => operatorInfo ? insertOperatorInfo(operatorInfo)(db) : null)
