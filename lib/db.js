...
function list (dbRoot) {
  return fs.promises.mkdir(dbRoot)
    .catch(() => {})
    .then(() => fs.promises.readdir(dbRoot))
}
...
function rotate (dbRoot) {
  ...
  return fs.promises.mkdir(dbRoot)
    .catch(() => {})
    .then(() => fs.promises.writeFile(path.resolve(dbRoot, dbName), ''))
}

function save (dbRoot, tx) {
  return fs.promises.appendFile(path.resolve(dbRoot, dbName), JSON.stringify(tx) + '\n')
}
...
function load (dbPath) {
  ...
  return fs.promises.readFile(dbPath, {encoding: 'utf8'})
    .then(f => {
      const recs = f.split('\n')
