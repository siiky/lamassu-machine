...
const BILL_ACCEPTING_STATES = ['billInserted', 'billsRead', 'acceptingHcm2Bills', 'acceptingBills',
...
Brain.prototype.determinePlatform = function determinePlatform () {
  ...
  if (deviceConfig.brain.hasSsuboard) return 'SSUBOARD'
  return 'UPBOARD'
}
...
Brain.prototype.selectBillValidatorClass = function selectBillValidatorClass () {
  if (commandLine.mockCashRecycler) return require('./mocks/hcm2/hcm2')
...
Brain.prototype.setupHardware = function setupHardware () {
  ...
  switch (hardware) {
    ...
    case 'SSUBOARD':
      return this.setupSsuboard()
    case 'UPBOARD':
      return this.setupUpboard()
  }
}
...
Brain.prototype.setupUpboard = function setupUpboard () {
  cp.exec('ntpq -c "host time.nist.gov"', {}, err => {
    if (err) console.log(err)
  })
}
...
Brain.prototype._connectedBrowser = function _connectedBrowser () {
  ...
  if (!this.trader || !this.trader.coins) {
    const rec = {
      ...
      screenOpts: this.trader ? this.trader.machineScreenOpts : null
    }
...
Brain.prototype._continueSmsCompliance = function () {
  ...
  const returnState = this.billValidator.name === 'HCM2'
    ? 'acceptingHcm2Bills'
    : this.tx.fiat.eq(0)
      ? 'acceptingFirstBill'
      : 'acceptingBills'
...
Brain.prototype.complianceAmount = function complianceAmount () {
  ...
  const lastRejectedBills = _.defaultTo(BN(0), this.lastRejectedBillsFiat)
...
function chooseBillDispenser (config) {
  ...
  switch (billDispenser) {
    ...
    default:
      throw new Error('Bill dispenser not loaded')
  }
}
...
Brain.prototype._firstBill = function _firstBill () {
  this.billValidator.name === 'HCM2'
    ? this._setState('acceptingHcm2Bills')
...
Brain.prototype._billsRead = function _billsRead (data) {
  const billValidator = this.billValidator
  ...
  if (billValidator.hasShutter) this.lastRejectedBillsFiat = BN(0)

  const bills = Tx.createBills(data, this.tx)
  this.insertBills(data)

  // Current inserting bills
  const currentBillsFiat = sumBills(bills)

  // Current transaction's fiat not including current bill
  const fiatBeforeBills = this.tx.fiat

  // Total fiat inserted including current bill
  const fiatAfterBills = fiatBeforeBills.add(currentBillsFiat)
  ...
  if (remainingFiatToInsert.lt(currentBillsFiat)) {
    billValidator.reject()
    this.lastRejectedBillsFiat = currentBillsFiat
  ...
  const amount = fiatBeforeBills.add(currentBillsFiat)
  const triggerTx = { fiat: amount, direction: this.tx.direction }
  ...
  this.browser().send({
    ...
    readingBills: currentBillsFiat.toNumber(),
  })
...
Brain.prototype.updateBillScreen = function updateBillScreen (blockedCustomer) {
  ...
  const state = this.billValidator.name === 'HCM2'
    ? 'acceptingHcm2Bills'
    : 'acceptingBills'

  return this.fastUpdateTx(Tx.billsUpdate(bills))
    .then(() => {
...
Brain.prototype._billStandby = function _billStandby () {
  if (this.state === 'acceptingHcm2Bills' || this.state === 'acceptingBills' || this.state === 'acceptingFirstBill') {
    this.enableBillValidator()
  }
}
...
Brain.prototype._uiCredit = function _uiCredit () {
  const tx = Tx.update(this.tx, Tx.billsUpdate(this.bills))
...
const sumBills = _.reduce((acc, value) => acc.add(value.fiat), BN(0))

/**
 * Clear the rejected bill and keep its amount as the lastRejectedBillsFiat
 *
 * @name clearBills
 * @function
 *
...
Brain.prototype.clearBills = function clearBills () {
  this.lastRejectedBillsFiat = sumBills(this.bills)
  this.bills = []
}

Brain.prototype.insertBills = function insertBills (bills) {
  console.assert(sumBills(this.bills).eq(0), "bill fiat is positive, can't start tx")
  this.bills = _.map(it => Tx.createBill(it.denomination, this.tx), bills)
}
...
Brain.prototype.hcm2BillFinish = function hcm2BillFinish (bills, blockedCustomer) {
  return this.fastUpdateTx(Tx.billsUpdate(bills))
    .then(() => this.completeBillHandling(blockedCustomer))
    .then(() => this._screenTimeout(() => this._sendCoins(), 5000))
}

Brain.prototype.hcm2Continue = function hcm2Continue () {
  return this.billValidator.cashCount()
    /*
     * TODO: If there are any destinations other than 'Escrow' (in particular,
     * 'Cash Slot'), we have to go into the alternative flow, to let the
     * customer take out their rejected money, etc.
     */
    .then(({ result }) => result['Escrow'])
    .then(bills => Promise.all([bills, this._billsRead(bills)]))
    .then(([bills, res]) => {
      if (res.success) {
        const billsObjArr = Tx.createBills(bills, this.tx)
        return this.billValidator.deposit()
          .then(() => this.hcm2BillFinish(billsObjArr))
      }

      if (res.func) {
        return res.func.bind(this)(...res.args)
      }
    })
}
...
Brain.prototype._doSendCoins = function _doSendCoins () {
  ...
  if (this.state !== 'acceptingBills' && this.state !== 'acceptingHcm2Bills' && !_.includes(this.state, complianceStates)) return
...
Brain.prototype._sendCoinsError = function _sendCoinsError (err) {
  ...
  if (err.statusCode === INSUFFICIENT_FUNDS_CODE) {
    this._transitionState('insufficientFunds', withdrawFailureRec)
    return this._timeoutToIdle(self.config.insufficientFundsTimeout || 60000)
  }
