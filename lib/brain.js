...
const BILL_ACCEPTING_STATES = ['billInserted', 'billsRead', 'acceptingShutterBills', 'acceptingBills',
...
Brain.prototype._continueSmsCompliance = function () {
  ...
  const returnState = this.billValidator.hasShutter()
    ? 'acceptingShutterBills'
    : this.tx.fiat.eq(0)
      ? 'acceptingFirstBill'
      : 'acceptingBills'
...
Brain.prototype._firstBill = function _firstBill () {
  const hasShutter = this.billValidator.hasShutter()
  this._setState(hasShutter ? 'acceptingShutterBills' : 'acceptingFirstBill')
  this.browser().send({
    billValidatorHasShutter: hasShutter,
    action: 'scanned',
...
Brain.prototype.updateBillsScreen = function updateBillsScreen (blockedCustomer) {
  ...
  const state = this.billValidator.hasShutter()
    ? 'acceptingShutterBills'
    : 'acceptingBills'
...
Brain.prototype._billStandby = function _billStandby () {
  if (this.state === 'acceptingShutterBills' || this.state === 'acceptingBills' || this.state === 'acceptingFirstBill') {
    this.enableBillValidator()
  }
}
...
Brain.prototype.shutterBillsFinish = function shutterBillsFinish (bills, blockedCustomer) {
  return this.fastUpdateTx(Tx.billsUpdate(bills))
    .then(() => this.completeBillHandling(blockedCustomer))
    .then(() => this._screenTimeout(() => this._sendCoins(), 5000))
}

Brain.prototype.shutterContinue = function shutterContinue () {
...
Brain.prototype._doSendCoins = function _doSendCoins () {
  ...
  if (this.state !== 'acceptingBills' && this.state !== 'acceptingShutterBills' && !_.includes(this.state, complianceStates)) return
