...
const BILL_ACCEPTING_STATES = ['billInserted', 'billRead', 'acceptingHcm2Bills', 'acceptingBills',
...
Brain.prototype._initBillValidatorEvents = function _initBillValidatorEvents () {
  ...
  if (this.billValidator.name !== 'HCM2') {
...
Brain.prototype._idle = function _idle (locale) {
  ...
  this.lastRejectedHcm2Fiat = BN(0)
...
Brain.prototype.complianceAmount = function complianceAmount () {
  ...
  const lastRejectedFiat = this.billValidator.name === 'HCM2'
    ? this.lastRejectedHcm2Fiat
    : this.lastRejectedBillFiat

  const lastRejectedBill = _.defaultTo(BN(0), lastRejectedFiat)
...
Brain.prototype._hcm2BillsRead = function _hcm2BillsRead (data) {
  const billValidator = this.billValidator
  ...
  const bills = this.buildHcm2Bills(data)

  const currentBills = _.reduce((acc, value) => acc.add(value.fiat), BN(0), bills)
  ...
  if (remainingFiatToInsert.lt(currentBills)) {
    billValidator.reject()
    this.lastRejectedHcm2Fiat = currentBills
  ...
  if (!isCompliant) {
    ...
    const nonCompliantTier = _.head(nonCompliantTiers)
    const idTier = nonCompliantTier === 'idCardData' || nonCompliantTier === 'idCardPhoto'
    if (idTier) return { success: false, func: this.transitionToVerificationScreen, args: [nonCompliantTier] }

    return { success: false, func: this.runComplianceTiers, args: [nonCompliantTiers] }
  }

  // verify if any non compliant manual trigger was scanned and end tx
  if (this.txBlockedByManualTrigger) {
    this.txBlockedByManualTrigger = false
    return { success: false, func: this.showBlockedCustomer, args: [] }
  }

  this.browser().send({ action: 'hcm2Continue' })

  billValidator.deposit(data)
...
Brain.prototype.updateBillScreen = function updateBillScreen (blockedCustomer) {
  ...
  return this.fastUpdateTx(Tx.billUpdate(bill))
    .then(() => {
...
Brain.prototype._uiCredit = function _uiCredit () {
  const tx = Tx.update(this.tx, Tx.billUpdate(this.bill))
...
Brain.prototype.buildHcm2Bills = function buildHcm2Bills (bills) {
  return _.map(it => Tx.createBill(it.denomination, this.tx), bills)
}
...
Brain.prototype.hcm2BillFinish = function hcm2BillFinish (bills, blockedCustomer) {
  return this.fastUpdateTx(Tx.billUpdateMultiple(bills))
    .then(() => this.completeBillHandling(blockedCustomer))
...
Brain.prototype.hcm2Continue = function hcm2Continue () {
  return this.billValidator.cashCount()
    .then(bills => Promise.all([bills, this._hcm2BillsRead(bills)]))
    .then(([bills, res]) => {
      if (res.success) {
        const billsObjArr = this.buildHcm2Bills(bills)
        return this.hcm2BillFinish(billsObjArr)
      }
...
Brain.prototype.ratesScreen = function () {
  ...
  this._transitionState('rates', { allRates: rates, ratesFiat: this.trader.locale.fiatCode, localeInfo: this.localeInfo })
}
