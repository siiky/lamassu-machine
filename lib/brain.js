...
Brain.prototype._hcm2BillsRead = function _hcm2BillsRead (data) {
  ...
  const bills = this.insertHcm2Bills(data)

  // Current transaction's fiat not including current bill
  const fiatBeforeBill = this.tx.fiat

  // Total fiat inserted including current bill
  const fiatAfterBill = fiatBeforeBill.add(_.reduce((acc, value) => acc.plus(value.denomination), BN(0), bills))
...
Brain.prototype.insertHcm2Bills = function insertHcm2Bills (bills) {
  const cryptoCode = this.tx.cryptoCode
  ...
  if (!serverVersion || semver.lt(serverVersion, '7.5.1-beta.0')) {
    ...
    return _.map(it => Tx.createBillDeprecated(it, exchangeRate, this.tx), bills)
  } else {
    return _.map(it => Tx.createBill(it, this.tx), bills)
  }
}
...
Brain.prototype.hcm2Continue = function hcm2Continue () {
  return this.billValidator.cashCount()
    .then(bills => this.billValidator.deposit(bills))
    .then(() => setTimeout(() => this._sendCoins(), 2000))
}
