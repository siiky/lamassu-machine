...
Brain.prototype._firstBill = function _firstBill () {
  const hasShutter = this.billValidatorHasShutter()
  this._setState(hasShutter ? 'acceptingFirstRecyclerBills' : 'acceptingFirstBill')
  this.browser().send({
...
Brain.prototype._billsRead = function _billsRead (data) {
  ...
  if (this.billValidatorHasShutter()) this.lastRejectedBillsFiat = BN(0)

  const bills = Tx.createBills(data, this.tx)
  this.insertBills(bills)
  // Current inserting bills
  const currentBills = getBillsFiatValue(this.bills)
...
Brain.prototype.updateBillsScreen = function updateBillsScreen (blockedCustomer) {
  ...
  if (!serverVersion || semver.lt(serverVersion, '7.5.0-beta.1')) {
    billsUpdate = Tx.billUpdateDeprecated(bills)
  } else {
    billsUpdate = Tx.billsUpdate(bills)
  }
  ...
  return this.fastUpdateTx(billsUpdate)
    .then(() => {
...
Brain.prototype._uiCredit = function _uiCredit () {
  ...
  if (!serverVersion || semver.lt(serverVersion, '7.5.0-beta.1')) {
    ...
    updatedBills = Tx.billsUpdate(this.bills)
  }
...
const getBillsFiatValue = _.reduce((acc, value) => acc.add(value.fiat), BN(0))
...
Brain.prototype.clearBills = function clearBills () {
  this.lastRejectedBillsFiat = getBillsFiatValue(this.bills)
  this.bills = []
}

Brain.prototype.insertBills = function insertBills (bills) {
  console.assert(getBillsFiatValue(this.bills).eq(0), "bill fiat is positive, can't start tx")
